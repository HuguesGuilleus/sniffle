...
func TestMyHandler(t *testing.T) {
	...
	assert.Equal(t, "\033[33m16:29:24 WARN___ [yolo]\033[0m\n", buff.String())
	buff.Reset()
	New(buff, slog.LevelDebug).Handle(ctx, r0)
	assert.Equal(t, "\033[33m16:29:24 WARN___ [yolo] src=f_test.go:3\033[0m\n", buff.String())
	...
	New(buff, slog.LevelWarn).
		...
		WithAttrs([]slog.Attr{
			slog.Any(" with", true),
			{Key: "service", Value: slog.StringValue("serv")},
		}).
		WithGroup("g2").
		Handle(ctx, r1)
	assert.Equal(t, "\033[33m16:29:24 WARN___ [serv|\"y\\\"ol\\\"o\"] g1.\" with\"=true_ g1.g2.int=-53\033[0m\n", buff.String())
	...
	assert.Equal(t, "\033[32m16:29:24 INFO+1_ [m] u=1234 f64=4.1 false=false nil=<nil> dur=2s now=2024-08-03T16:29:24 gr=[ k1=1 k2=2 ]\033[0m\n", buff.String())
}
